...
if (Numeration == 3) && ( (TypePosition_Rev_R3 == 1) || ...
        (TypePosition_Rev_R3 == 2) || (TypePosition_Rev_R3 == 33) )
...
for i = 1 : 3 % Additional
    hold on
    plot(YForecast(1,i),PForecast_Rev(1,i),'cs','linewidth',2); % Additional
end

if (Spark(7) ~= 0) && (TypePosition_Rev_R3 ~= -1) && (Typ_Pri(Spark(7)) < PForecast_Rev(1,Cut_Number))
    X_2 = xlim;
    New_lineP = PForecast_Rev(1,Cut_Number) + (PForecast_Rev(1,Cut_Number) + Typ_Pri(Spark(7))) / 2  + 4 * 10^(-(Digit-1)); % TP = 150
    New_lineP_D = Typ_Pri(Spark(7)) - abs(PForecast_Rev(1,Cut_Number) - Typ_Pri(Spark(7))) / 2 + 4 * 10^(-(Digit-1)); % SL = 50
    ...
    hold on
    plot(TimeClose_Rev_R3,PriceClose_Rev_R3,'b^','linewidth',1.5); % Additional
elseif (Spark(7) ~= 0) && (TypePosition_Rev_R3 ~= -1) && (Typ_Pri(Spark(7)) > PForecast_Rev(1,Cut_Number))
    X_2 = xlim;
    New_lineP = PForecast_Rev(1,Cut_Number) - (PForecast_Rev(1,Cut_Number) + Typ_Pri(Spark(7))) / 2  - 4 * 10^(-(Digit-1)); % TP = 150
    New_lineP_D = Typ_Pri(Spark(7)) + abs(PForecast_Rev(1,Cut_Number) - Typ_Pri(Spark(7))) / 2 - 4 * 10^(-(Digit-1)); % SL = 50
    ...
    hold on
    plot(TimeClose_Rev_R3,PriceClose_Rev_R3,'b^','linewidth',1.5); % Additional
elseif (TypePosition_Rev_R3 == 100)
    X_2 = xlim;
    New_lineP = PForecast_Rev(1,Cut_Number) - (PForecast_Rev(1,Cut_Number) + Typ_Pri(Spark(7))) / 2  - 4 * 10^(-(Digit-1)); % TP = 150
    New_lineP_D = Typ_Pri(Spark(7)) + abs(PForecast_Rev(1,Cut_Number) - Typ_Pri(Spark(7))) / 2 - 4 * 10^(-(Digit-1)); % SL = 50
...
if (Numeration == 3) && ( (TypePosition_Rev_R3(ii-1,1) == 1) || ...
        (TypePosition_Rev_R3(ii-1,1) == 2) || (TypePosition_Rev_R3(ii-1,1) == 33) )
...
if (Spark(ii+5) ~= 0) && (TypePosition_Rev_R3(ii-1,1) ~= -1) && (Typ_Pri(Spark(ii+5)) < PForecast_Rev(ii-1,Cut_Number(ii-1,1)))
    X_2 = xlim;
    New_lineP(ii-1,1) = PForecast_Rev(ii-1,Cut_Number(ii-1,1)) + (PForecast_Rev(ii-1,Cut_Number(ii-1,1)) + Typ_Pri(Spark(ii+5))) / 2  + 4 * 10^(-(Digit-1)); % TP = 150
    New_lineP_D(ii-1,1) = Typ_Pri(Spark(ii+5)) - abs(PForecast_Rev(ii-1,Cut_Number(ii-1,1)) - Typ_Pri(Spark(ii+5))) / 2 + 4 * 10^(-(Digit-1)); % SL = 50
    ...
    line(X_2,Y_4,'Color',[.3 .9 .9]);
    hold on
    plot(TimeClose_Rev_R3(ii-1,1),PriceClose_Rev_R3(ii-1,1),'b^','linewidth',1.5); % Additional
elseif (Spark(ii+5) ~= 0) && (TypePosition_Rev_R3(ii-1,1) ~= -1) && (Typ_Pri(Spark(ii+5)) > PForecast_Rev(ii-1,Cut_Number(ii-1,1)))
    X_2 = xlim;
    New_lineP(ii-1,1) = PForecast_Rev(ii-1,Cut_Number(ii-1,1)) - (PForecast_Rev(ii-1,Cut_Number(ii-1,1)) + Typ_Pri(Spark(ii+5))) / 2  - 4 * 10^(-(Digit-1)); % TP = 150
    New_lineP_D(ii-1,1) = Typ_Pri(Spark(ii+5)) + abs(PForecast_Rev(ii-1,Cut_Number(ii-1,1)) - Typ_Pri(Spark(ii+5))) / 2 - 4 * 10^(-(Digit-1)); % SL = 50
    ...
    hold on
    plot(TimeClose_Rev_R3(ii-1,1),PriceClose_Rev_R3(ii-1,1),'b^','linewidth',1.5); % Additional
elseif (TypePosition_Rev_R3(ii-1,1) == 100)
    X_2 = xlim;
    New_lineP(ii-1,1) = PForecast_Rev(ii-1,Cut_Number(ii-1,1)) - (PForecast_Rev(ii-1,Cut_Number(ii-1,1)) + Typ_Pri(Spark(ii+5))) / 2  - 4 * 10^(-(Digit-1)); % TP = 150
    New_lineP_D(ii-1,1) = Typ_Pri(Spark(ii+5)) + abs(PForecast_Rev(ii-1,Cut_Number(ii-1,1)) - Typ_Pri(Spark(ii+5))) / 2 - 4 * 10^(-(Digit-1)); % SL = 50
...
writetable(T,'Final_Result_6.csv')
writetable(T,'Final_Result_6.xlsx')
%type 'Final_Result.csv'

Sequence = table(SequenceZeroOne');
writetable(Sequence,'SequenceZeroOne_6.csv')

Sequence_Sel_R1 = table(SequenceZeroOne_Sel_R1');
writetable(Sequence_Sel_R1,'SequenceZeroOne_Sel_R1_6.csv')
Sequence_Rev_R1 = table(SequenceZeroOne_Rev_R1');
writetable(Sequence_Rev_R1,'SequenceZeroOne_Rev_R1_6.csv')

Sequence_Sel_R2 = table(SequenceZeroOne_Sel_R2');
writetable(Sequence_Sel_R2,'SequenceZeroOne_Sel_R2_6.csv')
Sequence_Rev_R2 = table(SequenceZeroOne_Rev_R2');
writetable(Sequence_Rev_R2,'SequenceZeroOne_Rev_R2_6.csv')

Sequence_Sel_R3 = table(SequenceZeroOne_Sel_R3');
writetable(Sequence_Sel_R3,'SequenceZeroOne_Sel_R3_6.csv')
Sequence_Rev_R3 = table(SequenceZeroOne_Rev_R3');
writetable(Sequence_Rev_R3,'SequenceZeroOne_Rev_R3_6.csv')
