...
m_1 = 3 * k;
M = 5000; % Resolution
m22 = 45000;%1759, 3500, 4127, 7913, 8972, 9536

NSZO = 0; % Sequence Index of Zero and One
disp('--------------------------------')
kk = input('Please enter a number from 1-22: '); % High=A(:,2); Low=A(:,3); Close=A(:,4)

tic
[A, Text, Row, PipValue, Digit] = Select_Excel(kk);
...
%         end
%         %----------------------------------------------
%         if (Max_PercentF >= 50) && (Deviation_F >= 100)
%             if x_maxF < x_minF
%                 TypePosition_F = 1; % Max_Percent
%                 FinalPrice_F = (Typ_Pri(Spark(7)) + First_PForecast) / 2;
%                 
%                 PiP = abs(FinalPrice_F - Typ_Pri(Spark(7))) * 10^(Digit-1);
%                 NetProfit = PiP - 4;
%                 PerPosition = NetProfit * PipValue;
%                 
%                 TimeClose = x_maxF;
%                 PriceClose = Typ_Pri(x_maxF);
%             else
%                 TypePosition_F = 2; % Deviation
%                 FinalPrice_F = Typ_Pri(Spark(7)) - abs(First_PForecast - Typ_Pri(Spark(7)));
%                 
%                 PiP = -abs(FinalPrice_F - Typ_Pri(Spark(7))) * 10^(Digit-1);
%                 NetProfit = PiP - 4;
%                 PerPosition = NetProfit * PipValue;
%                 
%                 TimeClose = x_minF;
%                 PriceClose = Typ_Pri(x_minF);
%             end
%             TypePos_MaxPer_Dev_F = abs( Typ_Pri(YForecast(1,1)) - Typ_Pri(Spark(7)) ) * 100 / ...
%                 abs( First_PForecast - Typ_Pri(Spark(7)) );
%             if Typ_Pri(Spark(7)) < Typ_Pri(YForecast(1,1))
%                 TypePos_F = {'Max_Percent'};
%             else
%                 TypePos_F = {'Deviation'};
%             end
%         elseif Max_PercentF >= 50
%             TypePosition_F = 1; % Max_Percent
%             TypePos_MaxPer_Dev_F = abs( Typ_Pri(YForecast(1,1)) - Typ_Pri(Spark(7)) ) * 100 / ...
%                 abs( First_PForecast - Typ_Pri(Spark(7)) );
%             if Typ_Pri(Spark(7)) < Typ_Pri(YForecast(1,1))
...
% plot(Spark(7),Typ_Pri(Spark(7)),'rh','linewidth',2) % Additional
% 
% if (Spark(7) ~= 0) && (TypePosition_F ~= -1) && (Typ_Pri(Spark(7)) < First_PForecast)
%     X_2 = xlim;
...
%     hold on
%     plot(TimeClose,PriceClose,'b^','linewidth',1.5); % Additional
% elseif (Spark(7) ~= 0) && (TypePosition_F ~= -1) && (Typ_Pri(Spark(7)) > First_PForecast)
%     X_2 = xlim;
%     Y_2 = [New_lineP, New_lineP];
%     Y_3 = [New_lineP_D, New_lineP_D];
%     Y_4 = [Hunt_Price - 4 * 10^(-(Digit-1)), Hunt_Price - 4 * 10^(-(Digit-1))];
    ...
    if Typ_Pri(Spark(ii+5)) < First_PForecast(ii-1,1) % Hunt Up
        ...
        for i = Spark(ii+5) : YForecast(ii-1,1) - 1
            if (((Typ_Pri(i) >= New_lineP(ii-1,1)) && (Typ_Pri(i+1) < New_lineP(ii-1,1))) ...
                ...
                for i2 = 1 : 3 % Additional
                    hold on
                    X_1 = [YForecast(ii-1,i2), YForecast(ii-1,i2)];
                    p5 = line(X_1,Y_1,'Color',[.5 .3 1.0]);%[.6 .1 .9]
                    hold on
                    p6 = plot(YForecast(ii-1,i2),PForecast(ii-1,i2),'ch','linewidth',2); % Additional
                end
                hold on
                ...
                if (Spark(ii+5) ~= 0) && (TypePosition_F(ii-1,1) ~= -1) && (Typ_Pri(Spark(ii+5)) < First_PForecast(ii-1,1))
                    X_2 = xlim;
                    ...
                    line(X_2,Y_4,'Color',[.3 .9 .9]);
                    hold on
                    plot(TimeClose(ii-1,1),PriceClose(ii-1,1),'b^','linewidth',1.5); % Additional
                elseif (Spark(ii+5) ~= 0) && (TypePosition_F(ii-1,1) ~= -1) && (Typ_Pri(Spark(ii+5)) > First_PForecast(ii-1,1))
                    X_2 = xlim;
                    Y_2 = [New_lineP(ii-1,1), New_lineP(ii-1,1)];
                    Y_3 = [New_lineP_D(ii-1,1), New_lineP_D(ii-1,1)];
                    Y_4 = [Hunt_Price(ii-1,1) - 4 * 10^(-(Digit-1)), Hunt_Price(ii-1,1) - 4 * 10^(-(Digit-1))];
                    ...
                    hold on
                    plot(TimeClose(ii-1,1),PriceClose(ii-1,1),'b^','linewidth',1.5); % Additional
                elseif (TypePosition_F(ii-1,1) == 100)
                    X_2 = xlim;
                    Y_2 = [New_lineP(ii-1,1), New_lineP(ii-1,1)];
                    Y_3 = [New_lineP_D(ii-1,1), New_lineP_D(ii-1,1)];
                    Y_4 = [Hunt_Price(ii-1,1) - 4 * 10^(-(Digit-1)), Hunt_Price(ii-1,1) - 4 * 10^(-(Digit-1))];
...
%         end
%         %----------------------------------------------
%         if (Max_PercentF(ii-1,1) >= 50) && (Deviation_F(ii-1,1) >= 100)
%             if x_maxF < x_minF
%                 TypePosition_F(ii-1,1) = 1; % Max_Percent
%                 FinalPrice_F(ii-1,1) = (Typ_Pri(Spark(ii+5)) + First_PForecast(ii-1,1)) / 2;
%                 
%                 PiP(ii-1,1) = abs(FinalPrice_F(ii-1,1) - Typ_Pri(Spark(ii+5))) * 10^(Digit-1);
%                 NetProfit(ii-1,1) = PiP(ii-1,1) - 4;
%                 PerPosition(ii-1,1) = NetProfit(ii-1,1) * PipValue;
%                 
%                 TimeClose(ii-1,1) = x_maxF;
%                 PriceClose(ii-1,1) = Typ_Pri(x_maxF);
%             else
%                 TypePosition_F(ii-1,1) = 2; % Deviation
%                 FinalPrice_F(ii-1,1) = Typ_Pri(Spark(ii+5)) - abs(First_PForecast(ii-1,1) - Typ_Pri(Spark(ii+5)));
%                 
%                 PiP(ii-1,1) = -abs(FinalPrice_F(ii-1,1) - Typ_Pri(Spark(ii+5))) * 10^(Digit-1);
%                 NetProfit(ii-1,1) = PiP(ii-1,1) - 4;
%                 PerPosition(ii-1,1) = NetProfit(ii-1,1) * PipValue;
%                 
%                 TimeClose(ii-1,1) = x_minF;
%                 PriceClose(ii-1,1) = Typ_Pri(x_minF);
%             end
%             TypePos_MaxPer_Dev_F(ii-1,1) = abs( Typ_Pri(YForecast(ii-1,1)) - Typ_Pri(Spark(ii+5)) ) * 100 / ...
%                 abs( First_PForecast(ii-1,1) - Typ_Pri(Spark(ii+5)) );
%             if Typ_Pri(Spark(ii+5)) < Typ_Pri(YForecast(ii-1,1))
%                 TypePos_F(ii-1,1) = {'Max_Percent'};
%             else
%                 TypePos_F(ii-1,1) = {'Deviation'};
%             end
%         elseif Max_PercentF(ii-1,1) >= 50
%             TypePosition_F(ii-1,1) = 1; % Max_Percent
%             TypePos_MaxPer_Dev_F(ii-1,1) = abs( Typ_Pri(YForecast(ii-1,1)) - Typ_Pri(Spark(ii+5)) ) * 100 / ...
%                 abs( First_PForecast(ii-1,1) - Typ_Pri(Spark(ii+5)) );
%             if Typ_Pri(Spark(ii+5)) < Typ_Pri(YForecast(ii-1,1))
...
%             PriceClose(ii-1,1) = Typ_Pri(x_minF);
%         else
%             TypePosition_F(ii-1,1) = 3; % No mode
...
T = table(CT_1, DT_1, ST_1, De_1, Extreme_distance_with_spark_1, Phase_change_time_1, Pr_1, ...
    ...
    New_lineP, New_lineP_D, Hunt_Price ); % Deviation_F, Max_PercentF
...
writetable(Sequence,'SequenceZeroOne.csv')
