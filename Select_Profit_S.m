function [XNUD_Forecast, Hunt_Forecast, SellBuy, TimeOpen, PriceOpen, TimeClose, ...
    ...
    = Select_Profit_S(Typ_Pri, YForecast, PForecast, Spark, NSZO, b_T_6, b_T_7, ...
    TimeClose, ii, Digit, MM, CASE, Stop_HD)
...
if NumberTime == 0
    if Typ_Pri(Spark) < PForecast % Hunt Up
        ...
        if CASE == 1 % R/R = 1
            New_lineP = PForecast + 4 * 10^(-(Digit-1)); % TP = 100
            New_lineP_D = Typ_Pri(Spark) - abs(PForecast - Typ_Pri(Spark)) + 4 * 10^(-(Digit-1)); % SL = 100
        elseif CASE == 2 % R/R = 1
            New_lineP = (PForecast + Typ_Pri(Spark)) / 2  + 4 * 10^(-(Digit-1)); % TP = 50
            New_lineP_D = Typ_Pri(Spark) - abs(PForecast - Typ_Pri(Spark)) / 2 + 4 * 10^(-(Digit-1)); % SL = 50
        elseif CASE == 3 % R/R = 3
            New_lineP = PForecast + (PForecast + Typ_Pri(Spark)) / 2  + 4 * 10^(-(Digit-1)); % TP = 150
            New_lineP_D = Typ_Pri(Spark) - abs(PForecast - Typ_Pri(Spark)) / 2 + 4 * 10^(-(Digit-1)); % SL = 50
        end
        
        if abs(Typ_Pri(Spark) - New_lineP_D) < Stop_HD
        if YForecast <= MM + 1 %******
            ...
            Hunt_Forecast = {'The stop line was activated.'};
            SellBuy = {'The stop line was activated.'};
