...
if (Numeration == 3) && ( (TypePosition_Sel_R1 == 1) || ...
        (TypePosition_Sel_R1 == 2) || (TypePosition_Sel_R1 == 33) )
...
if (Spark(7) ~= 0) && (TypePosition_Sel_R1 ~= -1) && (Typ_Pri(Spark(7)) < PForecast(1,Cut_Number))
    X_2 = xlim;
    New_lineP = PForecast(1,Cut_Number) + 4 * 10^(-(Digit-1)); % TP = 100
    New_lineP_D = Typ_Pri(Spark(7)) - abs(PForecast(1,Cut_Number) - Typ_Pri(Spark(7))) ...
        + 4 * 10^(-(Digit-1)); % SL = 100
    ...
    hold on
    plot(TimeClose_Sel_R1,PriceClose_Sel_R1,'b^','linewidth',1.5); % Additional
elseif (Spark(7) ~= 0) && (TypePosition_Sel_R1 ~= -1) && (Typ_Pri(Spark(7)) > PForecast(1,Cut_Number))
    X_2 = xlim;
    New_lineP = PForecast(1,Cut_Number) - 4 * 10^(-(Digit-1)); % TP = 100
    New_lineP_D = Typ_Pri(Spark(7)) + abs(PForecast(1,Cut_Number) - Typ_Pri(Spark(7))) ...
        - 4 * 10^(-(Digit-1)); % SL = 100
    ...
    hold on
    plot(TimeClose_Sel_R1,PriceClose_Sel_R1,'b^','linewidth',1.5); % Additional
elseif (TypePosition_Sel_R1 == 100)
    X_2 = xlim;
    New_lineP = PForecast(1,Cut_Number) - 4 * 10^(-(Digit-1)); % TP = 100
    New_lineP_D = Typ_Pri(Spark(7)) + abs(PForecast(1,Cut_Number) - Typ_Pri(Spark(7))) ...
        - 4 * 10^(-(Digit-1)); % SL = 100
...
if (Numeration == 3) && ( (TypePosition_Sel_R1(ii-1,1) == 1) || ...
        (TypePosition_Sel_R1(ii-1,1) == 2) || (TypePosition_Sel_R1(ii-1,1) == 33) )
...
if (Spark(ii+5) ~= 0) && (TypePosition_Sel_R1(ii-1,1) ~= -1) && (Typ_Pri(Spark(ii+5)) < PForecast(ii-1,Cut_Number(ii-1,1)))
    X_2 = xlim;
    New_lineP(ii-1,1) = PForecast(ii-1,Cut_Number(ii-1,1)) + 4 * 10^(-(Digit-1)); % TP = 100
    New_lineP_D(ii-1,1) = Typ_Pri(Spark(ii+5)) - abs(PForecast(ii-1,Cut_Number(ii-1,1)) - Typ_Pri(Spark(ii+5))) ...
        + 4 * 10^(-(Digit-1)); % SL = 100
    ...
    line(X_2,Y_4,'Color',[.3 .9 .9]);
    hold on
    plot(TimeClose_Sel_R1(ii-1,1),PriceClose_Sel_R1(ii-1,1),'b^','linewidth',1.5); % Additional
elseif (Spark(ii+5) ~= 0) && (TypePosition_Sel_R1(ii-1,1) ~= -1) && (Typ_Pri(Spark(ii+5)) > PForecast(ii-1,Cut_Number(ii-1,1)))
    X_2 = xlim;
    New_lineP(ii-1,1) = PForecast(ii-1,Cut_Number(ii-1,1)) - 4 * 10^(-(Digit-1)); % TP = 100
    New_lineP_D(ii-1,1) = Typ_Pri(Spark(ii+5)) + abs(PForecast(ii-1,Cut_Number(ii-1,1)) - Typ_Pri(Spark(ii+5))) ...
        - 4 * 10^(-(Digit-1)); % SL = 100
    ...
    hold on
    plot(TimeClose_Sel_R1(ii-1,1),PriceClose_Sel_R1(ii-1,1),'b^','linewidth',1.5); % Additional
elseif (TypePosition_Sel_R1(ii-1,1) == 100)
    X_2 = xlim;
    New_lineP(ii-1,1) = PForecast(ii-1,Cut_Number(ii-1,1)) - 4 * 10^(-(Digit-1)); % TP = 100
    New_lineP_D(ii-1,1) = Typ_Pri(Spark(ii+5)) + abs(PForecast(ii-1,Cut_Number(ii-1,1)) - Typ_Pri(Spark(ii+5))) ...
        - 4 * 10^(-(Digit-1)); % SL = 100
...
writetable(Sequence_Sel_R1,'SequenceZeroOne_Sel_R1.csv')
Sequence_Rev_R1 = table(SequenceZeroOne_Rev_R1');
writetable(Sequence_Rev_R1,'SequenceZeroOne_Rev_R1.csv')

Sequence_Sel_R2 = table(SequenceZeroOne_Sel_R2');
writetable(Sequence_Sel_R2,'SequenceZeroOne_Sel_R2.csv')
Sequence_Rev_R2 = table(SequenceZeroOne_Rev_R2');
writetable(Sequence_Rev_R2,'SequenceZeroOne_Rev_R2.csv')

Sequence_Sel_R3 = table(SequenceZeroOne_Sel_R3');
writetable(Sequence_Sel_R3,'SequenceZeroOne_Sel_R3.csv')
Sequence_Rev_R3 = table(SequenceZeroOne_Rev_R3');
writetable(Sequence_Rev_R3,'SequenceZeroOne_Rev_R3.csv')
